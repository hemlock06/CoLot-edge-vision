...
def features(e: Event) -> np.ndarray:
    ...
    return np.array(
        [dur, np.log1p(dur), hour, e.flicker, float(e.registered)], np.float32
    )
...
class ParkingAnomalyDetector:
    def __init__(self, cfg: AnomalyCfg = AnomalyCfg(), seed: int = 20231016):
        ...
        self.iforest = IsolationForest(
            contamination=cfg.contamination, n_estimators=200, random_state=seed
        )
        self._fitted = False
    ...
    def predict(self, events: list[Event]) -> list[Flag]:
        ...
        scores = (
            self.iforest.score_samples(X) if self._fitted else np.zeros(len(events))
        )
        ml_anom = (
            self.iforest.predict(X) == -1
            if self._fitted
            else np.zeros(len(events), bool)
        )
        out = []
