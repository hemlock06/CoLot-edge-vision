...
from kev.adaptive import AdaptiveSensor, brightness_features, feature_vector, rule_label
import random
...
def _gen(n_scenes, seed, variant="A"):
    ...
    for _ in range(n_scenes):
        ...
        for env in ENVS:
            ...
            img = np.clip(
                img.astype(np.float32) + rng.normal(0, 9, img.shape), 0, 255
            ).astype(np.uint8)
...
def eval_classifier(seed=SEED):
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "adaptive_confusion.png", dpi=130)
    plt.close(fig)
    return dict(
        acc_clf=acc_clf,
        acc_rule=acc_rule,
...
def eval_tradeoff(n_frames=600, seed=SEED + 1):
    ...
    envs = list(ENV_PRIOR)
    probs = np.array(list(ENV_PRIOR.values()))
    probs /= probs.sum()

    mode_cnt = Counter()
    ocr_adp = 0
    recoverable = captured = 0
    for _ in range(n_frames):
        ...
        if car:
            ...
            img = apply_env(_asphalt(640, 480, rng), env, rng)
            motion = float(rng.uniform(0, 0.008))
        d = sensor.step(img, motion=motion)
        mode_cnt[d.mode] += 1
        ocr_adp += int(d.run_ocr)
        if car and RECOVERABLE[env]:
            recoverable += 1
            captured += int(d.run_ocr and d.mode != "skip")
    ...
    saves = [
        power_saved({"rgb_full": 1.0, "rgb_boost": 1.15, "ir": ir, "skip": sk}, oc)
        for ir in (0.4, 0.55, 0.7)
        for sk in (0.02, 0.05, 0.1)
        for oc in (0.5, 0.8, 1.2)
    ]
    res = dict(
        n_frames=n_frames,
        ocr_calls_reduced=1 - ocr_adp / n_frames,  # ★ 실측 1차 지표
        coverage_retained=captured / max(1, recoverable),
        recoverable=recoverable,
        power_saved_default=default,
        power_saved_min=min(saves),
        power_saved_max=max(saves),
    ...
    ax[0].set_ylim(0, 100)
    ax[0].set_ylabel("%")
    ax[0].set_title("추론 호출 ↓ · 캡처 유지")
    ax[1].bar(["전력 절감"], [default * 100], color="#9aa1ac")
    ax[1].errorbar(
        [0],
        [default * 100],
        yerr=[
            [default * 100 - res["power_saved_min"] * 100],
            [res["power_saved_max"] * 100 - default * 100],
        ],
        fmt="o",
        color="#1F2A3A",
        capsize=6,
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "adaptive_tradeoff.png", dpi=130)
    plt.close(fig)
    return res
...
if __name__ == "__main__":
    ...
    print(
        f"  회수가능 캡처 유지     {t['coverage_retained'] * 100:5.1f}%  (recoverable={t['recoverable']})"
    )
    print(
        f"  전력 절감(가정 의존)   {t['power_saved_default'] * 100:.1f}%  [범위 {t['power_saved_min'] * 100:.0f}~{t['power_saved_max'] * 100:.0f}%]"
    )
