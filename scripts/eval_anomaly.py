...
def main():
    ...
    cr = int(len(evs_r) * 0.5)
    tr_r, te_r = evs_r[:cr], evs_r[cr:]
    y_r = [e.label for e in te_r]
    pr_rule = [
        f.pred for f in ParkingAnomalyDetector(AnomalyCfg(), seed=SEED).predict(te_r)
    ]
    pr_ml = [
        f.pred
        for f in ParkingAnomalyDetector(AnomalyCfg(), seed=SEED)
        .fit([e for e in tr_r if e.label == "normal"])
        .predict(te_r)
    ]
    res["random_fault"] = dict(
        rule_only=binary(y_r, pr_rule),
        rule_ml=binary(y_r, pr_ml),
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["무단점유(미등록)", "센서고장"])
    ax.set_ylim(0, 108)
    ax.set_ylabel("재현율 (%)")
    ax.legend()
    ax.set_title("유형별 이상 재현율 — ML이 센서고장을 보강")
    fig.tight_layout()
    fig.savefig(FIGS / "anomaly_recall.png", dpi=130)
    plt.close(fig)
    ...
    cm = confusion_matrix(yt, [simplify(p) for p in pred_ml], labels=PRED_LABELS)
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "anomaly_confusion.png", dpi=130)
    plt.close(fig)

    print("=== 이진 이상탐지 ===")
    print(
        f"  룰만      P={res['rule_only']['precision']:.3f} R={res['rule_only']['recall']:.3f} F1={res['rule_only']['f1']:.3f}"
    )
    print(
        f"  룰+IForest P={res['rule_ml']['precision']:.3f} R={res['rule_ml']['recall']:.3f} F1={res['rule_ml']['f1']:.3f}"
    )
    print("=== 유형별 재현율 (룰만 → 룰+ML) ===")
    for t in TYPES:
        print(
            f"  {t:12s} {res['recall_rule'][t] * 100:5.1f}% → {res['recall_ml'][t] * 100:5.1f}%"
        )
    print(
        f"test={res['n_test']} anomalies={res['n_anom']} "
        f"무단평균체류={res['unauthorized_mean_dwell_min']}"
    )
    rf = res["random_fault"]
    print("=== A2: 비설계 랜덤고장 (자기충족 반박) ===")
    print(
        f"  룰만      P={rf['rule_only']['precision']:.3f} R={rf['rule_only']['recall']:.3f} F1={rf['rule_only']['f1']:.3f}"
    )
    print(
        f"  룰+IForest P={rf['rule_ml']['precision']:.3f} R={rf['rule_ml']['recall']:.3f} F1={rf['rule_ml']['f1']:.3f}"
    )
    print(
        f"  센서고장 재현율(룰 → 룰+ML): {rf['fault_recall_rule'] * 100:.1f}% → {rf['fault_recall_ml'] * 100:.1f}%"
    )
