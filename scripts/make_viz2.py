...
def adaptive_decisions():
    ...
    for ax, env in zip(axes.ravel(), ENVS):
        ...
        ax.set_title(
            f"실제:{ENV_KO[env]} → 판단:{ENV_KO.get(d.env, d.env)} {ok}\n"
    ...
    fig.suptitle(
        "③ 휘도·악천후 환경적응 — 각 환경에서 센서가 내린 판단",
        fontsize=14,
        fontweight="bold",
    )
    fig.tight_layout()
    fig.savefig(FIGS / "adaptive_decisions.png", dpi=130)
    plt.close(fig)
...
def anomaly_sessions():
    ...
    for e, f in zip(evs, flags):
        ...
        ax[1].scatter(
            ...
            c=LAB_COLOR[e.label],
            alpha=0.6,
            edgecolors=("black" if flagged else "none"),
            linewidths=0.7,
        )
    ax[1].set_xscale("log")
    ax[1].set_xlabel("체류시간 (분, log)")
    ax[1].set_ylabel("센서 토글수 (flicker)")
    ax[1].set_title("세션 분포 — 고장(짧음/김/토글)이 정상과 분리")
    ax[1].axvline(2.0, ls="--", c="#888", lw=1)
    ax[1].axvline(18 * 60, ls="--", c="#888", lw=1)
    ax[1].axhline(20, ls="--", c="#888", lw=1)

    fig.suptitle(
        "② 불법주차 이상탐지 — 데이터(이미지 아님): 점유×예약 세션",
        fontsize=14,
        fontweight="bold",
    )
    fig.tight_layout()
    fig.savefig(FIGS / "anomaly_sessions.png", dpi=130)
    plt.close(fig)
