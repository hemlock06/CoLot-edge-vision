...
def main():
    ...
    det = OnnxYolo(DATA / "runs" / "plate" / "weights" / "best_int8.onnx", imgsz=320, conf=0.25)
    ...
    res = dict(n=len(imgs),
               ...
               env_dist=envs, rows=rows)
    (DATA / "real_metrics.json").write_text(
        json.dumps(res, ensure_ascii=False, indent=2), encoding="utf-8")

    # 몽타주
    n = min(12, len(imgs)); ncol = 4; nrow = (n + ncol - 1) // ncol
    ...
    for ax, r in zip(np.atleast_1d(axes).ravel(), rows[:n]):
        ...
        ax.set_title(f"③{r['env']} · 검출 {r['nbox']} · OCR:{r['ocr'] or '—'}", fontsize=8)
    fig.suptitle("실데이터 검증 — 합성 학습 파이프라인을 실 번호판 이미지에 적용", fontweight="bold")
    fig.tight_layout(); fig.savefig(FIGS / "real_validation.png", dpi=130); plt.close(fig)
