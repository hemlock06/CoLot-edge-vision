...
def main(n=40):
    ...
    for cond in CONDS:
        ...
        for t in texts:
            ...
            em += int(pred == t); ca += char_accuracy(pred, t)
    ...
    ax.set_xticks(x); ax.set_xticklabels(["맑음", "비", "안개", "눈"])
    ax.set_ylim(0, 108); ax.set_ylabel("%"); ax.legend()
    ax.set_title("악천후별 번호판 OCR 정확도 — 적응 정책 필요성의 근거")
    fig.tight_layout(); fig.savefig(FIGS / "weather_ocr.png", dpi=130); plt.close(fig)

    print("=== 악천후별 OCR ===")
    for c in CONDS:
        print(f"  {c:6s} 완전일치={res[c]['exact']*100:5.1f}%  문자정확도={res[c]['char']*100:5.1f}%")
