...
def main():
    d = load_split()
    Xtr = np.array([x for x, _, _ in d["train"]])
    ytr = [c for _, c, _ in d["train"]]
    Xva = np.array([x for x, _, _ in d["val"]])
    yva = [c for _, c, _ in d["val"]]
    print(
        f"train {len(ytr)} · val {len(yva)} · 분포 val:", dict(collections.Counter(yva))
    )
    ...
    res = dict(
        n_train=len(ytr),
        n_val=len(yva),
        val_dist=dict(collections.Counter(yva)),
        synthetic_on_real_acc=acc_syn,
        synthetic_on_real_recall=rec_syn,
        real_calibrated_acc=acc_real,
        real_calibrated_recall=rec_real,
        real_cv_4cls_mean=float(cv.mean()),
        real_cv_4cls_std=float(cv.std()),
    ...
    axb.bar(
        x - w / 2,
        [rec_syn[c] for c in cls],
        w,
        label="합성 ③ → 실(전이)",
        color="#C0504D",
    )
    axb.bar(
        x + w / 2, [rec_real[c] for c in cls], w, label="실-보정 ③", color="#1F6FB2"
    )
    axb.set_xticks(x)
    axb.set_xticklabels(cls, rotation=20, fontsize=9)
    axb.set_ylim(0, 1.05)
    axb.set_ylabel("재현율")
    axb.legend(fontsize=8, loc="upper right")
    axb.set_title(
        f"실 도로장면 ③ — 전이 acc {acc_syn:.2f} → 실보정 {acc_real:.2f}\n"
        f"(야간 전이 {rec_syn.get('low_light', 0):.2f} ↔ 주간/날씨 약함)",
        fontsize=10,
    )
    ...
    fig.suptitle(
        "③ 환경분류 실데이터 검증 (BDD100K 실 주행) — 저조도는 전이, 날씨는 합성 전용",
        fontweight="bold",
        fontsize=11,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(FIGS / "real_adaptive.png", dpi=130)
    plt.close(fig)
