...
def main():
    ...
    for k in KIND_KO:
        ...
        if ak:
            by_kind[k] = dict(
                ...
                in_occupancy_rate=float(np.mean([a.lead > 0 for a in ak])),  # 주차 중 탐지
                mean_violation_dur=float(np.mean([a.lead + a.delay for a in ak])))  # 맥락
    res = dict(n_alerts=len(alerts), by_kind=by_kind,
               in_occupancy_rate=float(np.mean([a.lead > 0 for a in alerts])) if alerts else 0.0)
    ...
    ax[0].set_yticks(range(len(sample))); ax[0].set_yticklabels([KIND_KO[a.kind] for a in sample], fontsize=8)
    ax[0].set_xlabel("시각 (시)"); ax[0].set_title("▼위반시작  ★경보(주차 중)  |출차 — 경보가 출차보다 앞섬")
    # (우) 탐지 지연(= grace, 위반 후 경보까지)
    ks = list(by_kind); delays = [by_kind[k]["mean_delay"] for k in ks]
    ax[1].bar([KIND_KO[k] for k in ks], delays, color=[KIND_COL[k] for k in ks])
    for i, k in enumerate(ks):
        ax[1].text(i, delays[i], f"{delays[i]:.0f}분", ha="center", va="bottom", fontsize=9)
    ax[1].set_ylabel("탐지 지연 (분, 위반→경보)"); ax[1].set_title("위반 후 이만큼 만에 경보 (배치는 출차 후)")
    fig.suptitle("② 실시간 조기 경보 — 주차 중 위반 탐지 (배치=출차 후 대비)", fontweight="bold")
    fig.tight_layout(); fig.savefig(FIGS / "streaming.png", dpi=130); plt.close(fig)

    print("=== 실시간 조기 경보 (정직 지표) ===")
    print(f"  총 경보 {res['n_alerts']}건 · 주차 중 탐지율 {res['in_occupancy_rate']*100:.0f}%")
    for k in by_kind:
        b = by_kind[k]
        print(f"  {KIND_KO[k]:16s} n={b['n']:3d}  탐지지연 {b['mean_delay']:6.1f}분  "
