...
def main():
    ...
    variants["PyTorch GPU\n(클라우드 FP32)"] = dict(
        size=size_mb(best),
        lat=torch_latency(yolo, sample, device=0),
        det=det_rate_torch(yolo, val),
        kind="cloud",
    )
    variants["PyTorch CPU\n(FP32)"] = dict(
        size=size_mb(best),
        lat=torch_latency(yolo, sample, device="cpu"),
        det=None,
        kind="mid",
    )
    ...
    del yolo
    gc.collect()
    torch.cuda.empty_cache()  # GPU 해제 후 onnx 단계

    print("       벤치마크 (onnx CPU)...")
    fp32 = OnnxYolo(onnx_fp32, imgsz=IMGSZ)
    int8 = OnnxYolo(onnx_int8, imgsz=IMGSZ)
    variants["ONNX CPU\n(FP32)"] = dict(
        size=size_mb(onnx_fp32),
        lat=fp32.latency_ms(sample),
        det=det_rate_onnx(fp32, val),
        kind="mid",
    )
    variants["ONNX CPU INT8\n(온디바이스 엣지)"] = dict(
        size=size_mb(onnx_int8),
        lat=int8.latency_ms(sample),
        det=det_rate_onnx(int8, val),
        kind="edge",
    )
    ...
    ax[0].bar(
        names,
        [variants[n]["size"] for n in names],
        color=[colors[variants[n]["kind"]] for n in names],
    )
    ax[0].set_title("모델 크기 (MB)")
    ax[0].tick_params(axis="x", labelsize=7.5)
    ax[1].bar(
        names,
        [variants[n]["lat"] for n in names],
        color=[colors[variants[n]["kind"]] for n in names],
    )
    ax[1].set_title("추론 지연 (ms/img)")
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "plate_quant.png", dpi=130)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "plate_samples.png", dpi=130)
    plt.close(fig)
    ...
    for n in names:
        ...
        print(
            f"  {n.splitlines()[0]:16s} size={v['size']:5.1f}MB  lat={v['lat']:6.1f}ms  {d}"
        )
    print("=== OCR e2e ===")
