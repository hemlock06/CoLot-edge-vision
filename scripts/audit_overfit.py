...
def gen(n, seed, variant, drop=None):
    ...
    for _ in range(n):
        ...
        for env in ENVS:
            ...
            if g.random() < 0.5: im = cv2.GaussianBlur(im, (3, 3), 0)
            im = np.clip(im.astype(np.float32) + g.normal(0, 9, im.shape), 0, 255).astype(np.uint8)
            X.append(feature_vector(brightness_features(im))[keep]); y.append(env)
    return np.array(X, np.float32), np.array(y)

def rf(seed=0): return RandomForestClassifier(n_estimators=200, max_depth=10, random_state=seed, n_jobs=-1)

# ── 데이터 (장면 단위 분리) ──
Xa, ya = gen(80, SEED, "A");      XaT, yaT = gen(35, SEED + 777, "A")   # A train / A test
Xb, yb = gen(80, SEED + 5, "B");  XbT, ybT = gen(35, SEED + 777, "B")   # B train / B test

# ── (1) 양방향 OOD ──
clfA = rf().fit(Xa, ya); clfB = rf().fit(Xb, yb)
...
ra = classification_report(yaT, clfA.predict(XaT), labels=ENVS, output_dict=True, zero_division=0)
rb = classification_report(ybT, clfA.predict(XbT), labels=ENVS, output_dict=True, zero_division=0)
for e in ENVS:
    gap = ra[e]["recall"] - rb[e]["recall"]
    flag = "  ⚠과적합 의심" if gap > 0.12 else ""
    log(f"  {e:12s} in-dist {ra[e]['recall']:.2f}  OOD {rb[e]['recall']:.2f}  격차 {gap:+.2f}{flag}")

# ── (4) 피처 중요도 (날씨 3종 위치) ──
log("\n=== (4) 피처 중요도 top8 ===")
imp = sorted(zip(FEATURE_ORDER, clfA.feature_importances_), key=lambda t: -t[1])
for f, v in imp[:8]: log(f"  {f:14s} {v:.3f}")
for f in ("speckle", "streak_coh"):
    log(f"  · {f}: {dict(imp)[f]:.3f}")

# ── (5) 피처 ablation — OOD '비' 재현율 ──
log("\n=== (5) Ablation: OOD '비' 재현율 (피처 제거 시) ===")
for name, drop in [("전체 15피처", []), ("− streak_coh(각도불변)", ["streak_coh"]),
                   ("− speckle", ["speckle"]),
                   ("기본 13(날씨피처 0)", ["speckle", "streak_coh"])]:
    Xa2, ya2 = gen(80, SEED, "A", drop=drop); XbT2, ybT2 = gen(35, SEED + 777, "B", drop=drop)
    c = rf().fit(Xa2, ya2)
    rep = classification_report(ybT2, c.predict(XbT2), labels=ENVS, output_dict=True, zero_division=0)
    log(f"  {name:22s} 비 OOD 재현율 {rep['rain']['recall']:.2f} · 전체 OOD {accuracy_score(ybT2, c.predict(XbT2)):.3f}")
