...
for j, (w, ko) in enumerate(WEATHER):
    img = (
        scene2.copy()
        if w == "clear"
        else add_weather(relight(scene2, "day_normal", rng), w, rng)
    )
    show(
        fig.add_subplot(gs[2, j]),
        img,
        ko,
        color=("#1E5631" if w == "clear" else "#C0392B"),
    )
show(
    fig.add_subplot(gs[2, 4]),
    relight(_asphalt(640, 480, rng), "day_normal", rng),
    "빈 노면(절전)",
    color="#6B7280",
)
fig.text(
...
fig.suptitle(
    "CoLot-edge 합성 시뮬레이션 이미지 — 번호판 · 조명 · 악천후",
    fontsize=14,
    fontweight="bold",
    y=0.96,
)
