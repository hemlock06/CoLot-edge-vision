...
**주차난·불법주차의 사회적 비용을 줄이는 무인 주차 관제가 목적이다.** 옥외 카메라는 야간·역광·악천후에 노출되고 클라우드 인식은 지연·전력에 취약하다 — 그래서 환경을 먼저 분류해 촬영을 정책화하고, 모델을 경량화해 **카스토퍼가 한글 번호판을 온디바이스로 직접 읽어** 무인 정산·불법주차 단속을 자동화한다.

코랏(CoLot) 스마트 카스토퍼의 엣지-비전 처리를 한 디바이스의 입출차 1회 흐름으로 재현했으며(공개/합성 데이터 기반), 세 모듈이 한 파이프라인으로 결합된다.
