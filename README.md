...
| **② 불법주차 이상탐지 + 실시간 경보 + 정산** | 원장 대조 **룰(P=1.0)** + IsolationForest 센서고장 안전망. **비설계 랜덤고장에서도 78.0→91.5** 입증. **실시간 경보**(주차 중 100% 탐지·지연=grace) + **분단위 정산 e2e** |
| **통합·정직성** | ③→①→② 단일 파이프라인 + CLI + 데모, **pytest 35**. 재검증 감사로 순환성·자기충족·가정 의존을 점검·보정(정직 수치) |
...
프레임 →│ ③ adaptive  휘도+dark-channel+채도 → 환경분류(RF,8) → 정책 │→ mode·run_ocr
        ...
        └──────────────────┬──────────────────────────────────────────┘
                           │ 점유 세션(start,end) + 앱 원장(회원 등록)
        ② anomaly  룰(원장대조) ⊕ IsolationForest(센서이상) → 무단/센서고장
```
...
|---|---|---|---|
| 룰만 (원장 대조) | 1.000 | 0.941 | 0.970 |
| 룰 + IsolationForest | 0.811 | 0.980 | 0.888 |

정직 표기: **F1은 룰만(0.970)이 더 높다** — ML은 센서고장 재현율 안전망(84.2→94.7), 정밀도 1.0→0.81 대가.
**비설계 랜덤고장에서도** 센서고장 재현율 **78.0→91.5** (ML 기여가 데이터 설계와 무관함 입증).
![anomaly](figs/anomaly_recall.png)

**실시간 조기 경보** (`kev/streaming.py`) — 이벤트-구동 스트림. **주차 중 탐지율 100%**
(출차 후 배치 아님), 탐지 지연=grace(무단 **3분**). ('168분 선행'은 위반 지속시간이라 폐기.)

**분단위 정산 e2e** (`kev/billing.py`) — 번호판(①)+점유(센서)→분단위 요금. 7분 주차
**240원**(경쟁사 30분 블록 1,200원 대비 절감) · 무단=과태료.
